...
export default function CategoryFormModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!trimmed) { setError("Tên danh mục không được trống."); return; }
    setError("");
            ...
            <form onSubmit={handleSubmit} className="p-6">
              <div className="flex items-start justify-between gap-4 mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-lg font-bold text-on-surface">Thêm danh mục</h2>
                    <p className="text-xs text-on-surface-variant mt-0.5">Nhập tên danh mục sản phẩm.</p>
                  </div>
                </div>
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-xs font-bold text-on-surface-variant mb-1.5 block">Tên danh mục <span className="text-red-500">*</span></label>
                  <input
                    required
                    autoFocus
                    value={name}
                    ...
                    placeholder="VD: Rau xanh, Trái cây hữu cơ"
                  />
                </div>
