...
import { Search, UserPlus, Lock, Unlock, Eye, ChevronLeft, ChevronRight } from "lucide-react";
import { ADMIN_USERS, ADMIN_ORDERS } from "../mocks";
    ...
    <div className="space-y-5 max-w-[1440px] mx-auto">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-on-surface-variant mt-0.5">{ADMIN_USERS.length} nguoi dung</p>
        </div>
        <button className="flex items-center gap-2 px-4 py-2.5 bg-primary text-white rounded-xl text-sm font-bold hover:brightness-110 transition-all self-start">
          <UserPlus className="w-4 h-4" /> Them nguoi dung
        </button>
      </motion.div>
      ...
      <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/20 overflow-hidden">
          ...
          <table className="w-full text-sm min-w-[800px]">
            <thead>
              <tr className="text-left text-on-surface-variant/70 text-xs border-b border-outline-variant/20 bg-surface-container-low/30">
                ...
                <th className="px-5 py-3 font-semibold">Don hang</th>
                <th className="px-5 py-3 font-semibold">Chi tieu</th>
              ...
              {filtered.map((user, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5 font-medium">{user.totalOrders}</td>
                    <td className="px-5 py-3.5 font-semibold text-primary">{user.totalSpent.toLocaleString()}₫</td>
                    <td className="px-5 py-3.5 text-on-surface-variant text-xs">{user.createdAt}</td>
                    <td className="px-5 py-3.5 text-right">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button className="p-1.5 rounded-lg hover:bg-surface-container text-on-surface-variant hover:text-amber-600 transition-colors" title={user.status === "active" ? "Khoa" : "Mo khoa"}>
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-outline-variant/20">
          <span className="text-xs text-on-surface-variant">Hien thi {filtered.length} / {ADMIN_USERS.length}</span>
      ...
      <UserDetailDrawer orders={ADMIN_ORDERS} user={selectedUser} onClose={() => setSelectedUser(null)} />
    </div>
  );
}
