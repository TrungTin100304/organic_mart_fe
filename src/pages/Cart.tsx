  ...
  return (
    <div className="max-w-7xl mx-auto px-6 md:px-10 py-10">
      <motion.nav {...fadeIn} className="flex items-center gap-2 mb-6 md:mb-10 text-on-surface-variant text-sm font-medium">
        ...
        <span className="text-primary font-bold">Gio hang</span>
      </motion.nav>
      ...
      {isLoading && <p className="text-on-surface-variant">Dang tai gio hang...</p>}
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-12 gap-8 md:gap-10 items-start">
            ...
            <div className="bg-white border border-outline-variant rounded-2xl overflow-x-auto shadow-sm">
              <table className="w-full min-w-[600px] text-left border-collapse">
                <thead className="bg-surface-container-low text-on-surface-variant text-[10px] font-bold uppercase tracking-[0.1em]">
                  <tr>
                    <th className="px-8 py-5">San pham</th>
                    <th className="px-6 py-5 text-center">Gia</th>
                    <th className="px-6 py-5 text-center">So luong</th>
                    <th className="px-6 py-5 text-right">Tam tinh</th>
                ...
                <tbody className="divide-y divide-outline-variant">
                  {cart?.items.length ? (
                    cart.items.map((item) => (
                      <tr key={item.productId} className="group hover:bg-surface-bright/50 transition-colors">
                        ...
                        <td className="px-6 py-8 text-center font-bold text-on-surface whitespace-nowrap">
                          {item.unitPrice.toLocaleString()}d
                        </td>
                        ...
                        <td className="px-6 py-8 text-right font-bold text-primary whitespace-nowrap">
                          {item.subtotal.toLocaleString()}d
                        </td>
                    ...
                    <tr>
                      <td className="px-8 py-10 text-center text-on-surface-variant" colSpan={5}>Gio hang dang trong.</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          <motion.aside initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="w-full lg:col-span-4 sticky top-24">
            <div className="bg-surface-container-high rounded-3xl p-8 border border-outline-variant shadow-lg ring-1 ring-white/50 backdrop-blur-sm">
              <h2 className="text-2xl font-bold text-on-surface mb-8 tracking-tight">Tom tat don hang</h2>
              <div className="space-y-6 pb-8 border-b border-outline-variant/50">
                <div className="flex justify-between items-center">
                  <span className="text-on-surface-variant font-medium">Tam tinh</span>
                  <span className="text-on-surface font-bold text-lg">{(cart?.totalPrice || 0).toLocaleString()}d</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-on-surface-variant font-medium">Phi van chuyen</span>
                  <span className="text-secondary font-bold text-lg">{shippingFee.toLocaleString()}d</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-on-surface-variant font-medium">Giam gia</span>
                  <span className="text-error font-bold text-lg">-{discount.toLocaleString()}d</span>
                </div>
              </div>

              <div className="flex justify-between items-end my-10">
                <span className="text-2xl font-bold text-on-surface tracking-tight">Tong cong</span>
                <div className="text-right">
                  <span className="text-3xl font-bold text-primary">{total.toLocaleString()}d</span>
                  <p className="text-[10px] text-on-surface-variant uppercase tracking-[0.2em] mt-1 font-bold">Da bao gom VAT neu co</p>
                </div>
              </div>
            ...
            <div className="mt-8 p-6 bg-secondary-container/50 border border-secondary-container rounded-2xl flex items-center gap-5 ring-1 ring-white/50">
              ...
              <p className="text-on-secondary-container text-sm font-bold leading-snug">Dam bao tuoi ngon hoac hoan tien trong 24 gio.</p>
            </div>
          </motion.aside>
        </div>
      )}
